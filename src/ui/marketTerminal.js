...
const stamp = (v) =>
  v
    ...
    : "No timestamp";
export function mountMarketTerminal({ onCompany, onLocation }) {
  let watch;
  try {
  ...
  nav.innerHTML = ["markets", "globe", "news", "energy"]
    .map(
  ...
  document.addEventListener("keydown", (e) => {
    if (["F1", "F2", "F3", "F4"].includes(e.key)) {
      e.preventDefault();
      desk(["markets", "globe", "news", "energy"][Number(e.key[1]) - 1]);
    }
  });
  ...
  function renderNews() {
    $("news-state").textContent = (news.status || "loading").toUpperCase();
    $("wire-query").textContent = news.query || "Loading headlines…";
    $("news-wire").innerHTML =
      (news.articles || [])
        .map((a, i) => {
          const c = classifyHeadline(a);
          return `<button class="news-row ${activeArticle === i ? "active" : ""}" data-article="${i}"><time>${esc(stamp(a.publishedAt))}</time><div><strong>${esc(a.title)}</strong><small>${esc(a.domain)} ${c.channels[0] ? " / " + esc(c.channels[0].label) : ""}</small></div><span>↗</span></button>`;
        })
        .join("") ||
      `<p class="feed-note">${news.status === "unavailable" ? "News provider unavailable. Refresh to retry." : "No recent headlines found for this query. Try a broader search."}</p>`;
  }
  function renderDrivers() {
    const q = quotes.get(selected.symbol),
      ...
      article =
        activeArticle === null
          ? null
          : classifyHeadline(news.articles[activeArticle]);
  ...
  root.addEventListener("click", (e) => {
    ...
    if (article) {
      activeArticle = Number(article.dataset.article);
      renderNews();
      renderDrivers();
    }
  });
