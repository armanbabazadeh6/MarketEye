...
export class WorldEventProvider {
  ...
  async load(key, url, normalize, { force = false } = {}) {
    ...
    if (!force && prior && this.now() - prior.retrievedMs < 300000)
      return {
        ...prior,
        status: prior.status === "stale" ? "stale" : "cached",
      };
    ...
    const work = (async () => {
      ...
      return prior
        ? { ...prior, status: "stale", error }
        : { key, events: [], status: "unavailable", retrievedAt: null, error };
    })();
  ...
  async refresh(company, { force = false } = {}) {
    ...
    const results = await Promise.all([
      ...
      ...locations.map((l) =>
        this.load(
          `weather:${l.id}`,
          `/api/weather-effects?latitude=${l.latitude}&longitude=${l.longitude}`,
          (p) => [normalizeWeather(p, l)],
          { force },
        ),
      ),
    ]);
    return {
      sources: results.map(({ events, ...status }) => status),
      events: results.flatMap((r) =>
        r.events.map((e) => ({ ...e, stale: e.stale || r.status === "stale" })),
      ),
