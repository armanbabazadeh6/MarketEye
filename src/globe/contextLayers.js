...
const definitions = [
  ...
  },
  {
    ...
    label: "Datacenters · OSM snapshot",
    load: () => import("../data/localLayers.js"),
  },
  {
    id: "local-firms",
    label: "Fires · NASA key",
    load: () => import("../data/localLayers.js"),
  },
  {
    id: "telegeography-submarine-cables",
    label: "Subsea cables · noncommercial",
    load: () => import("../data/localLayers.js"),
  },
];
export function attachContextLayers(globe, container) {
  ...
  container.innerHTML = definitions
    .map(
      (d) =>
        `<label class="layer"><input type="checkbox" data-context-layer="${d.id}"/>${d.label}<small id="layer-state-${d.id}"></small></label>`,
    )
    .join("");
  manager.subscribe(() => {
    for (const layer of manager.getAll()) {
      ...
      if (label) {
        label.textContent = layer.enabled ? layerFeedState(layer.stats) : "";
        label.title =
          layer.stats?.error ||
          layer.stats?.lastError ||
          `${layer.stats?.count ?? 0} records`;
      }
    }
  });
    ...
    (input) =>
      (input.onchange = async () => {
        const definition = definitions.find(
            (d) => d.id === input.dataset.contextLayer,
          ),
          label = document.getElementById(`layer-state-${definition.id}`);
        input.disabled = true;
        label.textContent = "Loading";
        try {
          if (!manager.layers.has(definition.id)) {
            const module = await definition.load();
            const layer = Array.isArray(module.default)
              ? module.default.find((l) => l.id === definition.id)
              : module.default;
            manager.register(layer);
          }
          await manager.setEnabled(definition.id, input.checked, {
            origin: "user",
          });
          input.checked = manager.isEnabled(definition.id);
          const state = manager.getAll().find((l) => l.id === definition.id);
          label.textContent = input.checked
            ? layerFeedState(state.stats)
            : "Off";
        } catch (error) {
          input.checked = false;
