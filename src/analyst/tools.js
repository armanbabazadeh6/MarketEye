...
export const TOOL_NAMES=['getCompanyExposure','getCompanyLocations','getSuppliers','getNearbyEvents','compareCompanies','showCompanyAssets','investigateEvent','generateCompanyBrief'];
export const toolDefinitions=TOOL_NAMES.map(name=>({type:'function',name,description:{getCompanyExposure:'Explain deterministic exposure, sources, uncertainty and structural dependencies.',getCompanyLocations:'List mapped company assets and approximate coordinates.',getSuppliers:'List documented supplier relationships and provenance.',getNearbyEvents:'Get events intersecting mapped assets.',compareCompanies:'Compare supported companies using the same event snapshot; identify coverage limitations.',showCompanyAssets:'Show me: move the globe to the selected company and show its assets.',investigateEvent:'Start a visual evidence investigation.',generateCompanyBrief:'Generate a sourced intelligence brief.'}[name],strict:true,parameters:{type:'object',properties:{ticker:{type:'string',enum:['NVDA','AAPL','TSLA']}},required:['ticker'],additionalProperties:false}}));
export function planLocally(question,ticker,companies) {
  const q=question.toLowerCase();
  const named=companies.find(c=>q.includes(c.ticker.toLowerCase())||q.includes(c.name.toLowerCase()));
  ticker=named?.ticker||ticker;
  const name=/compare|which compan/.test(q)?'compareCompanies':/investigat/.test(q)?'investigateEvent':/show|fly|take me/.test(q)?'showCompanyAssets':/brief|report/.test(q)?'generateCompanyBrief':/supplier/.test(q)?'getSuppliers':/location|factor|manufactur.*where/.test(q)?'getCompanyLocations':/earthquake|weather|event/.test(q)?'getNearbyEvents':'getCompanyExposure';
...
export function summarizeTool(name,result) {
  ...
  if(name==='getNearbyEvents')return result.events.length?result.events.slice(0,5).map(e=>`${e.exposure.explanation}\n${e.event.sourceUrl}`).join('\n\n'):'No correlated events in the available snapshot. This does not establish safe operations. Review source coverage.';
  if(name==='showCompanyAssets')return result.success?'Company footprint shown on the globe.':'Globe movement was unavailable or cancelled.';
  if(name==='investigateEvent')return result.success?'Investigation completed. The evidence timeline and brief are ready.':'Investigation cancelled or globe unavailable.';
