...
function renderTab(){
  if(activeTab==='analyst'){renderAnalyst();return;}
...
function selectCompany(ticker){cancelInvestigation();selected=companies.find(c=>c.ticker===ticker)||selected;selectedEvent=null;chatHistory=[];snapshot={sources:[],events:[]};assessment=assessCompany(selected,[]);renderCompany();renderIntelligence();globe?.showCompany(selected);globe?.showEvents([]);globe?.flyTo(selected.focus);$('search-results').innerHTML='';$('company-search').value='';void refreshSources();}
...
renderCompany();
try{globe=await createMarketGlobe(({kind,record})=>kind==='location'?selectLocation(record):selectEvent(record),message=>$('map-status').textContent=message);globe.showCompany(selected);globe.flyTo(selected.focus,1600000,0);}
...
function renderPath(event,exposure){
  ...
  if(!graph.nodes.length){$('impact-status').textContent='STRUCTURAL DEPENDENCY';return;}
...
function renderIntelligence(){
  ...
  const ranked=[...snapshot.events].sort((a,b)=>Number(relatedIds.has(b.id))-Number(relatedIds.has(a.id))||b.severity-a.severity);
...
async function refreshSources(force=false){
  ...
  finally{if(generation===requestGeneration){$('refresh').disabled=false;$('refresh').textContent='↻ Refresh sources';}}
}
function selectEvent(event){
  if(!event)return;selectedEvent=event;cancelInvestigation();globe?.flyTo(event);
...
function renderInvestigation(){
  let panel=$('investigation-panel');if(!panel){panel=document.createElement('section');panel.id='investigation-panel';panel.className='investigation-panel';document.querySelector('.globe-area').append(panel);}
