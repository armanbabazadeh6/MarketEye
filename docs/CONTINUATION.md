...
4. Run `npm run test:marketeye`, `npm test`, `npm run build` and, with the server running, `npm run qa:marketeye`, `npm run qa:terminal`, `npm run qa:research` and `npm run qa:integrated`.
5. Read `docs/BUILD_LOG.md` for working checkpoints. Version 0.3 adds the research notebook, monitor/comparison, command line, official NWS alerts, regional screening and combined dossier export.
...
- `src/ui/newsroom.js` and `src/research/store.js` own saved research. Backup/import must round-trip, reject executable URLs and preserve corrupt storage for recovery. Never replace a failed read with an automatic empty write.
- `src/ui/marketMonitor.js` compares daily series on common dates. Returns are price returns, not total returns. `src/markets/client.js` preserves last-good data with stale flags through local network failures.
- `src/events/officialAlerts.js` preserves NWS CAP certainty/onset/expiry. Alerts and headline-region matches do not enter the exposure score. Do not interpret a bounding-box center as a facility incident location.
- `BRIEF` exports prices, headline evidence and the existing geographic brief. `ANALYST` and `INV` bridge to existing bounded tools and camera tours.
...
2. Extend the implemented official NWS alerts with global official warnings and operator-confirmed operating status. Add facility polygons and defensible hazard footprints before interpreting damage.
...
- Real Yahoo quotes/charts/symbol search, Google News RSS and NWS alerts worked on 2026-09-11. Market, globe, research and integrated browser suites passed, including mobile and simulated provider outages. Ordinary unit suite: 2,856 passed, 5 skipped, plus two Node-24-only allocation checks excluded locally.
